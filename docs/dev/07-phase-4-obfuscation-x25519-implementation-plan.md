...
**Status:** 2026-07-21 Task 1-6 已全部完成并通过独立 RED/GREEN（或既有行为 first-run GREEN）、spec review、code/security review、focused/module/full reactor 门禁；Phase 4 已封板，无开放 P0/P1，下一候选为 Phase 5 Obfuscation Hybrid，等待用户确认。
...
实际 RED（2026-07-21）：JDK 17 focused test compilation 退出 1，全部错误且唯一原因是 `ObfuscationX25519SignedSender`、`ObfuscationX25519SignedReceiver` 及其嵌套 API 尚不存在；三组新测试自身没有语法或依赖错误。
...
- [x] **Step 4: 实现 signed Receiver 与认证门 tests**
...
- [x] **Step 5: 跑 focused、unsigned/public regression 与 module gate**
...
实际 GREEN（2026-07-21）：新增 sender 4 tests、receiver 6 tests、多收件人 E2E 2 tests，共 12 tests；计划列出的 signed/unsigned obfuscation 与 public signed regression 共 10 suites、69 tests 全绿。JDK 17 protocol dependency reactor 为 54 suites、549 tests，提交前 full reactor 为 65 suites、587 tests，均为 0 failure、0 error、0 skipped。真实 BC 三收件人生成 8-entry signed wire，binary/text/empty、任一收件人、无关收件人和 unknown signer 路径均通过。
...
实际双审（2026-07-21）：独立 spec review 与 code/security review 均为 P0=0、P1=0，未发现生产协议、密码学或认证漏洞；确认 final recipients 进入 AAD/binding、actual Ed25519 public key 派生 kid、exact received segments 验签、binding-before-resolver、record/actual kid 双绑定、错误语义、统一 `finally` 清理和 borrowed handles 正确。四类非阻塞测试证据缺口追加为 Deferred P2 第 17-20 项，不阻断 Demo 主链。本闭环仅提交 signed flow、fixture、三组测试、Task 5 证据和 P2 登记，排除 `docs/README.md`，也不自动 push。
...
- [x] **Step 1: bucket、payload 与首中尾真实 E2E**
...
实际证据（2026-07-21）：首条命令因新 fixture 多余右括号停在 test compilation，修正测试语法后首次有效行为运行直接 GREEN。unsigned 与 signed 分别用 `3→8`、`9→16`、`17→32` 三个代表场景分布 binary/text/empty payload，共 6 个参数化真实 BC E2E；32-entry wire 分别只提供最终 wire 顺序中首/中/尾真实 recipient handle 均恢复同一 payload，unrelated handle 为 `NOT_FOR_ME`。每条 wire 的全部 entry 均验证 16-byte rid、40-byte encryptedKey、无 `ek`，并由 strict parser、重算 AAD、binding 与 GCM 成功共同锁定最终 recipients 顺序。

- [x] **Step 2: authenticated tamper 矩阵**
...
实际证据（2026-07-21）：新增 8 个真实 tamper E2E，覆盖合法随机/低阶/全零 EPK、stale AAD、duplicate rid、selected encryptedKey 且 no-fallback、recipients reorder、iv/ciphertext/tag、真实 CEK 重加密后的 unsigned/signed binding，以及 signed signature/unknown signer/exact protected/payload segment。严格 EPK JSON/语义分类由本 Task 的 parser/canonical suites 覆盖；全部失败均通过 result holder 证明未释放 payload/identity。

- [x] **Step 3: randomness 与 writer/strict regression**
...
实际证据（2026-07-21）：randomness 1 test 首次有效运行即 GREEN，使用同一目标 private handle 对两条消息各自 EPK 重做派生，确认 target rid 各出现恰好一次，并证明 EPK、IV、target rid、ciphertext 全部新鲜。Writer/parser/canonical 三套新增回归首次运行即 GREEN，共 75 tests；现有生产 writer/parser 行为已满足 8-entry round-trip、EPK/rid canonical Base64URL、条件字段、双侧长度、bucket 与错误优先级要求，因此本 Step 没有修改生产代码。

- [x] **Step 4: 运行 focused 阶段六组合矩阵**
...
实际 focused（2026-07-21）：计划列出的 11 suites、98 tests，0 failure、0 error、0 skipped；obfuscation signed/unsigned 与 public X25519/Hybrid signed/unsigned 六组合全部通过真实 wire 回归。
...
实际门禁（2026-07-21）：JDK 17 protocol dependency reactor 为 56 suites、568 tests；fresh `mvn -q clean test` 为 67 suites、606 tests，均为 0 failure、0 error、0 skipped。测试总数较阶段起点 513 增加 93；`git diff --check` 通过，没有新增 `@Disabled`，Task 6 没有生产改动，`docs/README.md` 内容 diff 仍为空且保持未暂存。
...
实际双审（2026-07-21）：final spec review 与 final code/security review 均 PASS，P0=0、P1=0、新增 P2=0。逐项确认 per-message EPK、同一 shared secret 的 rid/KEK、bucket/decoy/shuffle、完整 Cartesian 扫描、constant-time comparator、tie-break、no-fallback、攻击者/本地错误分离、AAD/GCM/binding/signature 门序、敏感数组与 borrowed handles，以及真实 E2E/tamper 进入预期认证门。
...
阶段闭环（2026-07-21）：Task 1-5 分别提交为 `8414dcc`、`b521ea9`、`15519d4`、`6de41d6`、`647774c`；Task 6 使用下述封板提交。Phase 4 累计 20 项 Deferred P2，均已写清影响与建议；没有把协议、密码学或认证正确性问题降级为 P2。overall plan 已更新为 Phase 4 完成，后续停止在 Phase 5 候选边界等待用户确认。
...
- [x] 同一消息只有一个新 X25519 epk，跨消息不复用。
- [x] 同一次 SS_ECC 派生 exact rid/ecc 与 KEK 参数。
- [x] 真实 recipient 都能 unwrap 同一 CEK。
- [x] 1/8/9/16/17/32 正确映射 8/8/16/16/32/32。
- [x] 诱饵与真实 X25519 entry wire 长度一致、无 ek。
- [x] 全部 rid 唯一；真实冲突拒绝，诱饵最多重采样 128 次。
- [x] final shuffle 在 AAD/binding/signature/GCM 前完成。
- [x] Receiver 拒绝 duplicate wire rid 和 duplicate local key。
- [x] 每个 local key 只 derive 一次，并比较全部 local × wire 组合。
- [x] 多命中按 (wire index, local input index) 选择。
- [x] selected unwrap 失败不 fallback。
- [x] empty/unrelated local keys 只有完整扫描后 NOT_FOR_ME。
- [x] attacker-controlled agreement/unwrap failure 具有相同 code/message/null cause。
- [x] X25519 handle/public-key、HKDF contract 与 unwrap 非 CryptoOperationException runtime failure 为 INTERNAL_ERROR 并保留存在的本地 cause；selected unwrap 的 CryptoOperationException/null/错误长度按冻结合同统一为 KEY_UNWRAP_FAILED。
- [x] unsigned 完全跳过 Ed25519。
- [x] signed 只在 binding 后解析可信 signer 并验 exact segments。
- [x] 任意失败不返回 payload/identity。
- [x] ephemeral handle、shared secret、rid、KEK、CEK、inner、GCM AAD 和签名临时数组按 ownership 清理。
- [x] borrowed X25519/Ed25519 handles 不关闭。
- [x] Obfuscation X25519 signed/unsigned 真实 wire E2E、tamper、randomness、strict matrix 全绿。
- [x] public 四组合、JDK 17 full reactor、reviews 和 diff gate 全绿。
- [x] 没有开放 P0/P1。
...
17. Task 5 Receiver 的 unknown signer、resolver null/throws、record kid mismatch 与 actual public-key mismatch 已验证错误语义和门序，但未逐分支用 recording providers 捕获 CEK、GCM AAD、decrypted inner、signing public snapshot 等 raw references 并断言清零。生产统一 `finally` 已经双审确认正确；影响是未来分支级清零回归可能漏报，建议 Demo 后补齐参数化生命周期测试。
18. Task 5 Sender 的部分清零断言直接对 tracking lists 使用 `allMatch`，未先断言预期引用数量，空集合可能形成测试假阳性，尤其不能独立证明 tracking random 捕获了 CEK/IV。生产清零与实际捕获路径当前已人工核对；建议测试 hardening 时先断言 success/对应失败路径的引用数量，再逐项断言全零。
19. Task 5 的 tracking Ed25519 signer 固定用 fixture handle 完成真实签名，没有记录并断言 production 传入的 private-key handle。当前生产源码确实传入 Request 的 borrowed handle，且不关闭它；影响是未来传错 handle 的回归可能漏报，建议 tracking provider 同时保存 received handle 并 `assertSame`。
20. Task 5 Receiver 的若干 resolver lambda 没有捕获并断言查询 kid。当前生产源码使用 strict inner 的 `signingKid` 查询，并继续执行 record kid 与 actual public-key kid 双校验；影响是未来 resolver 查询参数改错时部分正例可能仍通过，建议统一 recording resolver 并断言 exact requested kid。
