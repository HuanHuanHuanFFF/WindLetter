...
**Status:** 2026-07-21 Task 1–6 已全部完成；八个 v1 profile、阶段负例与 fresh full reactor 均通过，Phase 5 已封板，等待用户确认 Phase 6。
...
```powershell
mvn -q -pl windletter-protocol -am "-Dtest=ObfuscationHybrid*Test,WindLetterEightProfilePayloadE2ETest,ObfuscationX25519*E2ETest,PublicX25519*E2ETest,PublicHybrid*E2ETest,JacksonOuterWireWriterTest,OuterWireCanonicalBase64UrlTest,OuterWireParserTest" "-Dsurefire.failIfNoSpecifiedTests=false" test
```
...
- Task 6 只修改测试、fixture 与阶段文档，没有改动生产实现；新增/扩展 6 个 representative bucket invocations、16 个八 profile payload round-trip、6 个 tamper、7 个 strict/writer 和 1 个 randomness test，共 36 tests。
- Hybrid E2E 覆盖 `1/8/9/16/17/32 → 8/8/16/16/32/32`；fixture 从最终 parsed wire 真实派生每个 pair 的匹配位置，按 shuffled wireIndex 排序后验证首/中/尾真实收件人。每条 wire 的全部 entry 均为 `rid16/ek1088/encrypted_key40`，rid 与 ek 全局唯一，且 `outer.aad` 等于最终 recipients 的 JCS。
- `WindLetterEightProfilePayloadE2ETest` 对 8 个 mode × algorithm × signing profile 分别运行 UTF-8 text 与含零字节/非法 UTF-8 byte 的 binary payload，16 条路径均使用真实 Bouncy Castle、正式 Sender/Receiver 和 JSON wire，并恢复 payload、message ID、timestamp 与认证身份。
- Tamper 门禁锁定：stale rid/ek/encrypted key 为 `AAD_MISMATCH` 且早于本地 pair 校验；swapped ek 为 `NOT_FOR_ME`；selected wrapped key 损坏为泛化 `KEY_UNWRAP_FAILED` 且 no fallback；重排并重算 AAD、IV/ciphertext/tag 修改为 `GCM_AUTH_FAILED`；authenticated wrong binding 为 `BINDING_FAILED`；unknown signer/signature flip 为 `SIGNATURE_INVALID`。异常不向调用者返回 Result，因此不释放 payload/identity 结果。
- Strict characterization 使用真实 Hybrid sender wire 覆盖 required/forbidden fields、`rid 15/17`、`ek 1087/1089`、`encrypted_key 39/41`、`epk.x 31/33`、四字段 padding/非法 alphabet/non-canonical trailing bits、非 bucket `7/9/15/17/31/33` 和 8-entry writer/parser 顺序保持。
- 相同业务输入连续发送两次时，EPK、IV、目标 rid、目标真实 ek 与 ciphertext 均不同；测试产生的 handle/context/material 和持有数组按路径关闭或清除。
- Corrected focused matrix 为 24 suites/170 tests；Task 6 六套件为 36 tests；protocol 及依赖为 69 suites/656 tests；指定 Microsoft OpenJDK 17.0.16 下 fresh `mvn -q clean test` 为 80 suites/694 tests，全部 0 failure、0 error、0 skipped。
- 没有删除既有测试或新增 `@Disabled/@Ignore`；final protocol/security、test-quality 与 phase-completeness 独立审查均为 P0=0、P1=0。既存 `docs/README.md` 行尾状态噪音继续排除，不属于本阶段提交。
...
- [x] rid 与 KEK 来自同一 entry、同一个 `SS_ECC || SS_PQ`。
- [x] 每个真实 entry 独立 ML-KEM encapsulation。
- [x] 单消息一个 EPK，全部真实项先于 padding/shuffle。
- [x] Hybrid decoy 三字段长度正确且来自 CSPRNG。
- [x] 只按完整 pair 判重；单 component 复用合法；无交叉拼接。
- [x] duplicate wire rid 在 local handle 前拒绝。
- [x] 正常输入严格 `N×M` decap/compare，命中无提前退出。
- [x] candidate crypto failure 全局锁存、dummy scan、zero unwrap。
- [x] tie-break 固定，selected unwrap 恰好一次且 no fallback。
- [x] wrong/swapped valid ek 与 provider failure 的语义不混淆。
- [x] strict → AAD → recovery → GCM → inner → binding → signature 门序正确。
- [x] signed exact segments 与可信实际公钥 kid 绑定。
- [x] borrowed handles 不关闭；owned secrets 全路径清除。
- [x] 八组合真实 crypto/JSON wire E2E 全绿。
- [x] 所有失败不返回 payload/identity。
- [x] final P0=0、P1=0。
...
21. **Task 6 Hybrid 专项 exact-segment 篡改深度**：Task 5 已证明 exact received/noncanonical segments 能按原字节验签，并覆盖 signature flip；Phase 4 同构测试覆盖 protected/payload segment 独立篡改。Task 6 未再把该负例绑定到 signed `9→16` bucket。影响仅为 profile × bucket 重复覆盖深度，不是已知验签绕过；Demo 后可在共享 matrix 中补齐。
22. **Task 6 swapped-ek E2E 计数深度**：真实 swapped-ek wire 已证明同步重算 AAD 后为 `NOT_FOR_ME`；`N×M` decap/compare 固定次数由 Task 3 component test 直接锁定，E2E 未重复插桩计数。影响是跨层重复证据深度，不影响当前完整扫描实现。
23. **Task 6 test helper 的跨 pair index 自证**：`realRecipientsInWireOrder` 会拒绝单 pair 的 0/多重命中，并按 wireIndex 排序；未额外以全局 Set 断言两个独立 pair 不会映射到同一 wireIndex。wire rid 唯一性、独立密钥和底层派生测试已保证现实路径；只影响测试 helper 的自包含证明，可在迁入 testkit 时补充。
