...
/** Complete receiver flow for the public/X25519ML-KEM-768/signed profile. */
public final class PublicHybridSignedReceiver {
    ...
    private static byte[] resolveSenderEncryptionPublicKey(
        ...
        if (callerValue == null || callerValue.length != X25519_PUBLIC_KEY_LENGTH) {
            throw invalid(
                    "resolved sender X25519 public key must contain exactly 32 bytes"
            );
        }
        ...
        if (!expectedKid.equals(X25519KeyId.derive(snapshot))) {
            clear(snapshot);
            throw invalid(
                    "resolved sender X25519 public key does not match protected kid"
            );
        }
        return snapshot;
    }
    ...
    private static void requireSignedProfile(WindLetter letter) {
        ...
        if (!profileMatches) {
            throw new ProtocolException(
                    ...
                    "message does not use the public/X25519ML-KEM-768/signed profile"
            );
        }
    }
            ...
            for (PublicHybridRecipientPrivateKeys privateKeys : recipientPrivateKeys) {
                if (privateKeys == null) {
                    throw new IllegalArgumentException(
                            "recipientPrivateKeys must not contain null"
                    );
                }
            }
