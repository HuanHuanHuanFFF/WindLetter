...
/** Complete receiver flow for the obfuscation/X25519ML-KEM-768/signed profile. */
public final class ObfuscationHybridSignedReceiver {

    private static final int X25519_PUBLIC_KEY_LENGTH = 32;
    private static final int MLKEM768_CIPHERTEXT_LENGTH = 1088;
    ...
    private final ObfuscationHybridCekRecovery cekRecovery;
    ...
    public ObfuscationHybridSignedReceiver(
            ObfuscationHybridCekRecovery cekRecovery,
            A256GcmCrypto gcm,
        ...
        try {
            ...
            try {
                cek = cekRecovery.recover(
                        epk,
                        letter.recipients(),
                        request.recipientPrivateKeys()
                );
            } catch (ProtocolException e) {
                throw e;
            } catch (RuntimeException e) {
                throw internal(
                        "failed to recover the obfuscation Hybrid recipient key",
                        e
                );
            }
            ...
            try (SignedInnerCodec.Decoded decoded = innerCodec.decode(decryptedInner)) {
                ...
                if (!inner.signingKid().equals(trustedKey.signingKid())) {
                    throw internal(
                            "trusted signing-key record does not match requested kid",
                            null
                    );
                }
                ...
                try {
                    ...
                    throw internal(
                            "trusted Ed25519 key record could not be validated",
                            e
                    );
                }
                if (!inner.signingKid().equals(actualKid)) {
                    throw internal(
                            "trusted Ed25519 public key does not match requested kid",
                            null
                    );
                }
    ...
    private static Epk requireProfile(WindLetter letter) {
        ...
        try {
            ...
            if (profileMatches) {
                for (RecipientEntry entry : letter.recipients()) {
                    ...
                    try {
                        if (ek == null || ek.length != MLKEM768_CIPHERTEXT_LENGTH) {
                            profileMatches = false;
                            break;
                        }
                    } finally {
        ...
        if (!profileMatches) {
            throw new ProtocolException(
                    ...
                    "message does not use the obfuscation/X25519ML-KEM-768/signed profile"
            );
        }
        return epk;
    }
    ...
    public record Request(
            ...
            List<ObfuscationHybridRecipientPrivateKeys> recipientPrivateKeys
    ) {
        public Request {
            if (senderSigningKeys == null) {
                throw new IllegalArgumentException(
                        "senderSigningKeys must not be null"
                );
            }
